...
class GGMLFile:
    def __init__(self, path: pathlib.Path) -> None:
        assert path.resolve().exists() and path.resolve().is_file()
        self._path = path
    ...
    def skip_vocab(
        ...
        for _ in range(n_vocab):
            ...
            _ = fp.read(word_len)
            if fmt != "ggmf" or version > 1:
                # For all formats newer than ggmfv1, the score is embedded in the vocab section.
                _ = fp.read(4)
    ...
    def read_tensor_descriptors(self, fp: BufferedReader) -> List[GGMLTensorDescriptor]:
        ...
        while len(fp.peek(4)[:4]) == 4:
            ...
            assert n_dims == 1 or n_dims == 2

            name_len = self.read_u32(fp)
            assert name_len <= 500

            shard_type = self.read_u32(fp)
            assert shard_type >= GGML_TYPE_F32 and shard_type <= GGML_TYPE_Q6_K
    ...
    for ggml_file in ggml_check_args.filenames:
        ...
        try:
            ...
            if ggml_check_args.verbose:
                print(json.dumps(asdict(ggml_struct)))
            elif ggml_check_args.verbose_pretty:
                print(json.dumps(asdict(ggml_struct), indent=4))
            else:
